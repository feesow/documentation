...
.. important::
   For the :guilabel:`Internal Transfers` card to appear on the :menuselection:`Overview` page of the
   :menuselection:`Inventory` app, the :guilabel:`Storage Locations` setting must be enabled. To do
   so, navigate to :menuselection:`Configuration --> Settings`, then enable the checkbox next to
...
Next, select a delivery order, receipt, or transfer from the corresponding page by clicking on it. A
...
.. important::
   ...
   To validate a receipt, click on the :guilabel:`Validate` button at the top left of the page. After
   doing so, the :guilabel:`Scrap` button appears at the top of the receipt.
